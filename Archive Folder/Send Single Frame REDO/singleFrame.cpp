...
const char *SERVER_IP = "172.17.140.7";
...
int main()
{
    ...
    // -----------[ START CAMERA STUFF ]----------- //
    // int customWidth = 1280;  // Set your desired width
    // int customHeight = 720;  // Set your desired height
    ...
    std::vector<uchar> imageData(cam0_frame.data, cam0_frame.data + cam0_frame.total() * cam0_frame.elemSize());
    // Send image size
    ...
    {
        int result = send(clientSocket, reinterpret_cast<const char *>(imageData.data()) + bytesSent, imageSize - bytesSent, 0);
        if (result <= 0)
        {
    ...
    //     printf("Sent image dimensions: %d x %d\n", rawImageWidth, rawImageHeight);
    //     printf("Sent image size: %zu bytes\n", rawImageWidth * rawImageHeight);
    //     printf("cv::Mat dimensions: %d x %d\n", cam0_frame.cols, cam0_frame.rows);
    //     printf("cv::Mat size: %zu bytes\n", cam0_frame.total() * cam0_frame.elemSize());

    //     // printf("Raw image size sent to WINDOWS machine : [ %d ] x [ %d ]\r\n", rawImageWidth, rawImageHeight);
    //     // printf("Number of bytes sent : [ %d ]\r\n", rawImageWidth * rawImageHeight);
    // }
    // // -----------[ END SEND RAW IMAGE SIZE ]----------- //

    // // -----------[ SEND RAW DATA TO WINDOWS MACHINE START ]----------- //
    ...
    // }
    // -----------[ SEND RAW DATA TO WINDOWS MACHINE END ]----------- //

    // // -----------[ RECIEVE THRESHOLD IMAGE FROM WINDOWS START ]----------- //
    ...
    // -----------[ RECIEVE THRESHOLD IMAGE FROM WINDOWS END ]----------- //
