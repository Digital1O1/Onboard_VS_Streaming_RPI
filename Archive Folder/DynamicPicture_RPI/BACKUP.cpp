...
int main()
{
    ...
    cap.set(cv::CAP_PROP_FOURCC, cv::VideoWriter::fourcc('M', 'J', 'P', 'G')); // Set to MJPEG for compatibility
    ...
    else
    {
        ...
        std::cout << "Time elapsed for RPI to SEND RAW IMAGE TO WINDOWS : " << recieveResolutionDuration.count() << " microseconds" << std::endl;
    }
    ...
    std::cout << "Time elapsed for RPI to RECIEVE THRESHOLDED IMAGE FROM WINDOWS : " << thresholdImageDuration.count() << " microseconds" << std::endl;
